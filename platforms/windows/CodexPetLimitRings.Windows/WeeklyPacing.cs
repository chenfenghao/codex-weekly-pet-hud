...
public static class WeeklyPacing
{
    public static PaceResult Calculate(double? remaining, long? resetAt, DateTimeOffset now)
    {
        if (remaining is null || !double.IsFinite(remaining.Value) || resetAt is null)
            return new(null, null, null, "等待数据", "粘贴周额度与重置时间，开始记录。", "#A1B2A7");
        DateTimeOffset reset;
        try { reset = DateTimeOffset.FromUnixTimeSeconds(resetAt.Value); }
        catch (ArgumentOutOfRangeException) { return new(null, null, null, "检查时间", "重置时间无效。", "#EDC788"); }
        var days = (reset - now).TotalDays;
        if (days <= 0) return new(100, null, null, "待确认重置", "重置时间已到，请重新粘贴最新额度。", "#EDC788");
        if (days > 7) return new(null, null, null, "检查时间", "重置时间超过 7 天，暂停预测。", "#EDC788");
        var left = Math.Clamp(remaining.Value, 0, 100);
        var elapsed = 7 - days;
        var time = elapsed / 7 * 100;
        var budget = Math.Min(left, left / days);
        if (left == 0) return new(time, time > 0 ? 100 / time : null, 0, "已耗尽", "额度已耗尽，等待重置后更新。", "#EE9D9D");
        if (elapsed * 86400 < 60) return new(time, null, budget, "刚刚开始", "周期刚开始，满 1 分钟后估算使用速率。", "#A1B2A7");
        var used = 100 - left;
        var ratio = used / time;
        var status = ratio < 0.85 ? "偏慢" : ratio <= 1 + 1e-9 ? "稳健" : ratio <= 1.3 + 1e-9 ? "偏快" : "超速";
        var color = ratio <= 1 + 1e-9 ? "#9BDFB5" : ratio <= 1.3 + 1e-9 ? "#EDC788" : "#EE9D9D";
        var prediction = "按当前平均速度，可用到重置日。";
        if (ratio > 1 && used > 0)
        {
            var end = reset.AddDays(-7 + elapsed * 100 / used);
            prediction = $"预计 {end.LocalDateTime.ToString("M月d日 ddd HH:mm", CultureInfo.GetCultureInfo("zh-CN"))} 耗尽\n比重置提前 {(reset - end).TotalHours:0.#} 小时";
        }
    ...
    public static string Countdown(long? resetAt, DateTimeOffset now)
    {
        ...
        try
        {
            ...
            return span <= TimeSpan.Zero ? "时间已到，请更新" : $"{(int)span.TotalDays}天 {span.Hours:00}:{span.Minutes:00}:{span.Seconds:00}";
        }
        catch (ArgumentOutOfRangeException) { return "无效时间"; }
    }
}
...
public static class WeeklyPasteParser
{
    ...
    {
        if (string.IsNullOrWhiteSpace(raw) || raw.Length > 12000) throw new FormatException("请只粘贴周额度与重置时间。");
        ...
        if (date.Success)
        {
            ...
            catch (ArgumentOutOfRangeException) { throw new FormatException("日期或时间无效，请检查后重试。"); }
        }
        else
        {
            ...
            if (clock.Success)
            {
                ...
                if (Part(1) > 7 || Part(2) > 23 || Part(3) > 59 || Part(4) > 59) throw new FormatException("倒计时无效。");
            ...
            else
            {
                ...
                if (!duration.Success) throw new FormatException("请同时粘贴重置时间，例如：2026年9月19日 16:12。");
                ...
                if (days > 7 || hours > 23) throw new FormatException("倒计时无效。");
                reset = now.AddDays(days).AddHours(hours);
            }
        }
        if (reset <= now || reset - now > TimeSpan.FromDays(7)) throw new FormatException("重置时间应在未来 7 天内。");
