...
**v1.4.0 双目标显示**：实际剩余额度下方，同时显示「匀速」和「下班」。

- **匀速应剩（此刻） = 距重置的剩余自然时间 ÷ 7 天 × 100%**，随时间变化。
- **下班应剩 = 今天下班后至重置的工作时长 ÷ 本周期全部工作时长 × 100%**，是当天收工时的计划余额，同一天内保持固定。两项目标均不随实际额度消耗改变。

设置 → 额度节奏算法中配置工作日、上下班时间和可选休息时段（15 分钟精度），例如周一到周六 07:00–21:00、不扣休息，每周 84 小时。下班前先重置时，下班目标以本次重置为截止，显示 0%；休息日显示「休息」目标。早于上班的下班时间表示跨午夜，工作日按班次开始日选择，当前夜班跨午夜后仍沿用该班下班时间。

「速率与预算按工作时段计算」只切换速率、耗尽预测与预算算法，不隐藏两个目标。工作模式跳过夜间、休息日和休息时段；详情中的 **今日建议 = 实际剩余额度 × 今天剩余工作小时 ÷ 本周期剩余工作小时**，今日以午夜为界，下班后为 0。关闭该选项时，速率按自然时间、预算按剩余天数计算。升级保留已有设置。

按本机时区、重置前固定 7 天窗口计算并裁切首尾班次。无有效工作时段时，下班目标显示「—」并提示检查作息，匀速目标仍可计算；无剩余工作时间时保留实际余额并暂停工作速率预测。修改作息会重新计算整个周期的工作目标。
...
<p>
  <img src="docs/images/capsule.png" alt="剩余额度、当前节奏、匀速应剩、下班应剩和重置雷达" width="285">
...
- **三行状态条**：默认约 190 × 72 DIP，可缩放；显示剩余、节奏、匀速应剩、下班应剩和重置雷达。
...
1. 在 [Releases](https://github.com/chenfenghao/codex-weekly-pet-hud/releases/latest) 下载 `Codex-Weekly-Pet-HUD-v1.4.0-Windows-x64.zip`。
...
| 匀速 | 按 7 天均匀使用，此刻理想的剩余比例 |
| 下班 | 按工作时段分配额度，今天下班时的计划剩余比例 |
| 详情中的建议 | 全天模式按剩余天数均摊；工作模式分配今天剩余工作时间可用的额度 |
| 速率 R | 本周期已用比例 ÷ 所选算法的时间进度（自然时间或工作时间） |
