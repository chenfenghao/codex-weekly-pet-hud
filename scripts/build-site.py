...
COPY = {
    'zh': {
        ...
        'figure': '真实界面 · 示例数据', 'pet': '跟随你现有的 Codex Pet', 'caption': '剩余额度 / 匀速应剩 / 下班应剩 / 重置雷达',
        'stats': [('190 × 72', '默认尺寸 · 可缩放'), ('1–60 min', '更新间隔 · 默认 5 分钟'), ('中文 / EN', '界面语言 · 随时切换')],
        'featureTitle': '需要的信息，刚好够用。',
        'features': [
            ('01', '看懂使用节奏', '同时显示此刻的匀速应剩，以及今天收工时的下班应剩。可设工作日、上下班与休息时间；速率、耗尽预测和详情预算支持全天或工作时段算法。'),
        ...
        'paceBody': '速率 R = 已用额度百分比 ÷ 所选模式的时间进度。例如时间已过 40%、额度已用 60%，R = 1.5，代表超速。',
        'paceLabels': ['偏慢', '稳健', '偏快', '超速'], 'paceNote': '按固定 7 天窗口估算；工作时段模式仅累计计划工作时间，今日建议按今天剩余工作时长分配。到达重置时间后等待真实数据。',
        'setupTitle': '解压，打开，就位。',
        'steps': [('完整解压下载包', '运行 CodexWeeklyPetHud.exe，保留旁边的 DLL 文件；无需单独安装 .NET。'), ('选择宠物或任务栏模式', '默认跟随 Codex Pet；也可从系统托盘进入设置，切换为任务栏显示。单击状态条查看详情。'), ('选好语言与输入方式', '设置 → 语言 / Language 切换中英文。自动读取不可用时，可以粘贴重置时间和剩余百分比。')],
        ...
        'privacy': '账户额度通过本机 Codex 登录读取，不记录 token；社区雷达使用独立的公开接口，不发送 OpenAI 凭据。宠物隐藏后雷达仍检查；宠物模式暂停读取账户额度，任务栏模式继续读取。网络失败会显示延迟，不会伪装成没有信号。',
        'community': '社区公告不代表你的账户已重置。雷达不会修改额度，也不会兑换重置次数。',
        'faq': [('没看到状态条？', '先检查系统托盘，并选择宠物或任务栏模式。系统面板展开时状态条会暂时隐藏。'), ('为什么显示连接延迟？', '检查网站访问与系统代理。公开雷达接口拒绝原生客户端时，可使用本机已安装的 Python 作为兼容方式；程序不自动安装 Python。'), ('支持 macOS 或 ARM64 吗？', '此分支目前只发布和验证 Windows x64，其他平台的上游代码保留作参考。')],
    ...
    'en': {
        ...
        'figure': 'Real interface · Sample data', 'pet': 'Follows your existing Codex Pet', 'caption': 'Quota left / Even-use target / Shift-end target / Reset radar',
        ...
        'features': [
            ('01', 'Understand your pace', 'See both the even-use balance for now and the planned balance at shift end. Configure weekdays, shifts and breaks; pace, exhaustion estimates and detail budgets support all-day or working-hours calculations.'),
        ...
        'paceBody': 'Pace R = quota percentage used ÷ time percentage elapsed in the selected mode. If 40% of the week has passed and 60% of quota is used, R = 1.5: Too fast.',
        'paceLabels': ['Slow', 'On track', 'Fast', 'Too fast'], 'paceNote': 'Uses a fixed seven-day window. Working-hours mode counts scheduled work and budgets for the work left today. Reaching reset does not automatically refill quota.',
        'setupTitle': 'Extract. Open. You are set.',
        'steps': [('Extract the entire archive', 'Run CodexWeeklyPetHud.exe and keep its DLL files alongside it. No separate .NET installation is needed.'), ('Choose Pet or Taskbar mode', 'Follow your Codex Pet by default, or choose Taskbar mode in Settings from the tray. Click the HUD for details.'), ('Choose your language and input', 'Use 设置 → 语言 / Language → English. If automatic quota reads fail, paste a reset time and remaining percentage instead.')],
        ...
        'privacy': 'Quota uses your local Codex login without logging tokens. Radar uses a separate public client without OpenAI credentials. Radar keeps checking with Pet hidden. Personal quota polling pauses in Pet mode and continues in Taskbar mode. Failed reads show a delay, not “no signal.”',
        ...
        'faq': [('No status bar?', 'Check Display mode in tray Settings. Pet mode needs an open Pet; Taskbar mode uses free space on the primary horizontal taskbar without pushing icons aside. System flyouts temporarily hide the HUD.'), ('Why does radar show Delayed?', 'Check site access and your system proxy. If public endpoints reject the native client, an installed Python interpreter can provide a compatibility fallback. The app does not install Python.'), ('macOS or ARM64?', 'This fork currently publishes and verifies Windows x64 only. Other platform code is retained from upstream for reference.')],
